...
_LOOP_SYSTEM_PROMPT = (
    ...
    "side-by-side comparison of the exact cited figures) for every "
    "candidate pair before including it, never because the category or "
    "the order of magnitude merely looks similar. A category's own total "
    "and one member's individual figure are NOT the same thing -- verify "
    "you are comparing the two values the question actually asked about, "
    "not two different figures that happen to share a label. Measured on "
    ...
    "RECORD IDENTITY: when you extract several fields about one named "
    "entity from a table with many similar rows (a vessel, a case, a "
    "property), verify every field you attach to it -- an ID number, a "
    "date, a code -- comes from THAT entity's own row, not a neighboring "
    "row for a different entity listed just above or below it. Measured "
    "on a real task: an answer correctly named the vessel and its report "
    "count, but the publication number field it gave (\"5/2023\") "
    "actually belonged to a different vessel listed in the adjacent row "
    "-- the right entity's row still has to be re-read for the specific "
    "field being reported, not assumed from a nearby line remembered "
    "while scanning past it. The same mistake happens sideways in a wide "
    "table with many similarly-named columns (year-over-year or "
    "sector-by-sector data repeated per state/row): count columns from "
    "the header out loud rather than assuming position from memory, and "
    "double check the column label directly above the cell you're "
    "reading. Measured on a real task: an answer consistently read the "
    "\"Commercial 2024\" column's values while labeling them as the "
    "\"Industrial Sector\" column -- every number it reported was real "
    "and correctly transcribed, just from the wrong column throughout. "
    "THRESHOLD / INEQUALITY CLAIMS: when a question asks which items are "
    "greater than, less than, or larger/smaller than a specific cutoff "
    "value, verify each comparison explicitly with `compute` (or an "
    "exact side-by-side of the two numbers) rather than judging by eye "
    "whether a value \"looks\" past the threshold -- values close to the "
    "cutoff are exactly where eyeballing fails. Measured on real tasks: "
    "one answer included a denomination whose actual figure (364.8M) "
    "was NOT below the stated 10% threshold (240M) it needed to clear; "
    "another labeled two features (1175 km, 1077 km) as \"larger than\" "
    "a 1500 km reference value when both are smaller. Do this check for "
    "every candidate near the boundary, not just the ones that already "
    "look like they qualify. "
    ...
    "wrong one. "
    "CITATION PRECISION: two more ways a citation can look present but "
    "fail to actually prove the claim. First, when a source has both a "
    "page that HOLDS the data (a table, an index, a list of entries) and "
    "a separate page that only DESCRIBES that data's format or how to "
    "read it, cite the page with the actual values -- a citation pointing "
    "at a format-description or \"how this index works\" page proves "
    "nothing about the specific numbers you claimed from it, even though "
    "it is the right source in general. Second, do not carry an "
    "exploratory or tangential source into your final citation list just "
    "because you opened it while researching -- a report or article that "
    "doesn't directly back a claim in your answer is noise, not evidence, "
    "and a clean citation list that only contains sources actually backing "
    "the answer beats an identical answer cluttered with extra sources "
    "that don't. "
    ...
    "is technically accurate. This includes CAPITALIZATION and DIACRITICS, "
    "not just wording: when the question asks for a name or label \"as it "
    "appears\" or \"exactly as written\" on a specific list or register, "
    "reproduce it byte-for-byte -- if the source prints \"WOLVERINE\" in "
    "block capitals or \"STORGLACIÄREN\" with an umlaut, do not "
    ...
    "and the answer still scored zero on that single casing change. The "
    "reverse case matters too: when the question explicitly asks you to "
    "CONVERT a name into \"ordinary title case\" (rather than preserve it "
    "exactly as printed), follow real title-case convention -- short "
    "prepositions, articles, and conjunctions (\"of\", \"the\", \"a\", "
    "\"in\", \"on\", \"and\") stay lowercase unless they are the first "
    "word, so \"The Banshees of Inisherin\" is correct title case, not "
    "\"The Banshees Of Inisherin\". Capitalizing every word is not title "
    "case. "
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "every clue was verified; (8) CASE/DIACRITIC FIDELITY -- if "
